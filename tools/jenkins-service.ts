...
export class JenkinsService {
  ...
  async startJob(app: string, branch: string): Promise<string> {
    ...
    const jobUrl = `${buildJobUrl('', app)}/buildWithParameters`;
    ...
    try {
      ...
      throw handleHttpError(error, `Failed to start job for app: ${app}`);
    }
  }
   ...
   */
  async stopJob(app: string, buildNumber: number): Promise<string> {
    if (!validateAppName(app)) {
      throw new Error('Invalid app name.');
    }

    const jobUrl = `${buildJobBuildUrl('', app, buildNumber)}/stop`;
    
    try {
      await this.client.post(jobUrl);
      return `Job stopped successfully for app ${app}, build ${buildNumber}`;
    } catch (error: any) {
      throw handleHttpError(error, `Failed to stop job for app: ${app}, build: ${buildNumber}`);
    }
  }
   ...
   */
  async getJobStepsStatus(app: string, buildNumber: number): Promise<BuildSteps> {
    if (!validateAppName(app)) {
      throw new Error('Invalid app name.');
    }

    const stepsUrl = `${buildJobBuildUrl('', app, buildNumber)}/wfapi/describe`;
    ...
    try {
      ...
      if (error.response?.status === 404) {
        throw new JenkinsError(`Build steps not found for app: ${app}, build: ${buildNumber}`);
      }
      throw handleHttpError(error, `Failed to get job steps for app: ${app}, build: ${buildNumber}`);
    }
  }
   ...
   */
  async getNodeStatus(app: string, buildNumber: number, nodeId: string): Promise<NodeStatus | PendingInputAction> {
    if (!validateAppName(app)) {
      throw new Error('Invalid app name.');
    }

    const nodeUrl = `${buildJobBuildUrl('', app, buildNumber)}/execution/node/${nodeId}/wfapi/describe`;
    ...
    try {
      ...
      if (nodeDetails.status === 'PAUSED_PENDING_INPUT') {
        return await this.getPendingInputActions(app, buildNumber);
      }
      
      return nodeDetails;
    } catch (error: any) {
      throw handleHttpError(error, `Failed to get node status for app: ${app}, build: ${buildNumber}, node: ${nodeId}`);
    }
  }
   ...
   */
  async getPendingInputActions(app: string, buildNumber: number): Promise<PendingInputAction> {
    if (!validateAppName(app)) {
      throw new Error('Invalid app name.');
    }

    const pendingInputUrl = `${buildJobBuildUrl('', app, buildNumber)}/wfapi/nextPendingInputAction`;
    
    try {
      const response: AxiosResponse<PendingInputAction> = await this.client.get(pendingInputUrl);
      return response.data;
    } catch (error: any) {
      throw handleHttpError(error, `Failed to get pending input actions for app: ${app}, build: ${buildNumber}`);
    }
  }
   ...
   */
  async getCoverageReport(app: string, buildNumber: number, packageName?: string, className?: string): Promise<CoverageReport | CoverageSummary> {
    if (!validateAppName(app)) {
      throw new Error('Invalid app name.');
    }
    ...
    try {
      const backendUrl = `${buildJobBuildUrl('', app, buildNumber)}/jacoco/jacoco.exec`;
      await this.client.get(backendUrl);
      // Si existe, procesar reporte backend
      return await this.getCoverageReportBackend(app, buildNumber);
    } catch (error: any) {
      // Si falla, intentar con frontend coverage
      try {
        const frontendReport = await this.getCoverageReportFrontend(app, buildNumber);
        ...
        throw handleHttpError(frontendError, `Failed to get coverage report for app: ${app}, build: ${buildNumber}`);
      }
    }
  }
   ...
   */
  async getCoverageReportLines(app: string, buildNumber: number, path: string): Promise<FileCoverage> {
    if (!validateAppName(app)) {
      throw new Error('Invalid app name.');
    }
    ...
    try {
      ...
      throw handleHttpError(error, `Failed to get coverage lines for app: ${app}, build: ${buildNumber}, path: ${path}`);
    }
  }
   ...
   */
  async getCoverageReportPaths(app: string, buildNumber: number): Promise<string[]> {
    if (!validateAppName(app)) {
      throw new Error('Invalid app name.');
    }
    ...
    try {
      ...
      throw handleHttpError(error, `Failed to get coverage paths for app: ${app}, build: ${buildNumber}`);
    }
  }
  ...
  async getGitBranches(app: string): Promise<string[]> {
    ...
    const url = `${buildJobUrl('', app)}/descriptorByName/net.uaznia.lukanus.hudson.plugins.gitparameter.GitParameterDefinition/fillValueItems?param=BRANCH_TO_BUILD`;
  ...
  private async getCoverageReportBackend(app: string, buildNumber: number): Promise<CoverageReport> {
    const jacocoUrl = `${buildJobBuildUrl('', app, buildNumber)}/jacoco/jacoco.exec`;
  ...
  private async getCoverageReportFrontend(app: string, buildNumber: number): Promise<CoverageReportFront> {
    const zipUrl = `${buildJobBuildUrl('', app, buildNumber)}/Coverage_20Unit_20Test_20Report/*zip*/Coverage_20Unit_20Test_20Report.zip`;
